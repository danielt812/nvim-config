...
-- stylua: ignore start
local blame     = function() vim.cmd("vertical Git blame --porcelain -- %:p") end
local log_file  = function() vim.cmd("Git log --follow -- %:p") end
local log_repo  = function() vim.cmd("Git log --oneline") end
local patch_log = function() vim.cmd("Git log --follow --patch -- %:p") end
...
do
  local ns = vim.api.nvim_create_namespace("mini_git_conflict")
  local group = vim.api.nvim_create_augroup("mini_git_conflict", { clear = true })
  ...
  local function darken_bg(hl_name, amount)
    ...
    if type(oklch) ~= "table" then return end
    oklch.l = math.max(0, oklch.l - amount)
    local hex = colors.convert(oklch, "hex", { gamut_clip = "chroma" })
  ...
  local function highlight_conflicts(buf)
    ...
    local extmark = function(line, col, opts) vim.api.nvim_buf_set_extmark(buf, ns, line, col, opts) end
    ...
    local function set_marker_hl(ln, bg_hl, marker_hl, label_hl)
      ...
      if marker_end < #line then
        extmark(ln - 1, marker_end, { end_col = #line, hl_group = label_hl, priority = 200 })
      end
    end
    ...
    for _, conflict in ipairs(conflicts) do
      ...
      if base then extmark(base[1] - 1, 0, { end_row = base[2] - 1, hl_group = "DiffDelete", hl_eol = true }) end
...
do
  ...
  local group = vim.api.nvim_create_augroup("mini_git_blame", { clear = true })
  ...
  local function format_blame(data, skip_consecutive)
    ...
    for _, entry in ipairs(data) do
      ...
      elseif entry.author == "Not Committed Yet" then
        ...
        table.insert(
          formatted,
          string.format("%s %s %s", entry.sha_short, pad_right(entry.date, max_date), entry.author)
        )
      end
      prev_sha = entry.sha
    end
  ...
  local function blame_cb(event)
    ...
    local function extmark(line, col, opts) vim.api.nvim_buf_set_extmark(buf, ns, line, col, opts) end
...
do
  ...
  local group = vim.api.nvim_create_augroup("mini_git_status", { clear = true })
